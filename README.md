...
ОТВЕТ:
Разница есть. 
В первом случае мы будем искать div в любом h1, а во втором случае мы находим h1 (первый), и у него ищем div (а таких может не оказаться).
...
ОТВЕТ:
GithubTest.checkSoftAssertions
...
ОТВЕТ:
HerokuappTest.checkActionsClickAndHold
и
HerokuappTest.checkDragAndDrop